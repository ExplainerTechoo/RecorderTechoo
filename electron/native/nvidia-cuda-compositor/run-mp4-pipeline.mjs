...
const ffmpegCommand = resolveToolCommand(["RecorderTechoo_FFMPEG_EXE"], "ffmpeg-static", "ffmpeg");
const ffprobeCommand = resolveToolCommand(["RecorderTechoo_FFPROBE_EXE"], "ffprobe-static", "ffprobe");
...
function shouldRaiseChildPriority() {
	return process.env.RecorderTechoo_NVIDIA_CUDA_EXPORT_HIGH_PRIORITY !== "0";
}
...
const outputPath = resolve(
	getArg("--output", join(scriptDir, "RecorderTechoo-nvdec-nvenc-mp4-output.mp4")),
);
...
function resolveNativeProbePath() {
	const configuredPath = process.env.RecorderTechoo_NVIDIA_CUDA_EXPORT_EXE;
	const platformArch = process.arch === "arm64" ? "win32-arm64" : "win32-x64";
	const candidates = [
		configuredPath,
		join(scriptDir, "build", "Release", "RecorderTechoo-nvidia-cuda-compositor.exe"),
		join(
			repoRoot,
			"electron",
			"native",
			"bin",
			platformArch,
			"RecorderTechoo-nvidia-cuda-compositor.exe",
		),
		// Backward-compatible legacy helper path while old work dirs are being retired.
		join(scriptDir, "build", "Release", "RecorderTechoo-nvdec-nvenc-probe.exe"),
	].filter(Boolean);
