...
async function executeWhisper(whisperExecutablePath: string, args: string[]) {
	try {
		...
		if (isMissingWindowsWhisperRuntimeDependency(error)) {
			throw new Error(
				"Whisper could not start because the Microsoft Visual C++ x64 Redistributable is missing. Install it from https://aka.ms/vc14/vc_redist.x64.exe, then restart RecorderTechoo.",
			);
		}
		throw error;
	}
...
export async function resolveWhisperExecutablePath(preferredPath?: string | null) {
	...
	throw new Error(
		`No Whisper runtime was found for ${process.platform}/${process.arch}. ` +
			"This RecorderTechoo build is missing its bundled caption runtime. Reinstall or update RecorderTechoo, or select a whisper-cli executable in Caption settings.",
	);
}
...
export async function generateAutoCaptionsFromVideo(options: {
	...
	const tempBase = path.join(
		app.getPath("temp"),
		`RecorderTechoo-captions-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
	);
