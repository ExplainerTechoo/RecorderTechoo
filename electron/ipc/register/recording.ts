...
import { ALLOW_RecorderTechoo_WINDOW_CAPTURE } from "../constants";
	...
	ipcMain.handle(
		...
		async (_, source: SelectedSource, options?: NativeMacRecordingOptions) => {
			...
			if (process.platform === "win32") {
				...
				try {
					...
					tempVideoPath = path.join(
						app.getPath("temp"),
						`RecorderTechoo-native-${timestamp}.mp4`,
					);
					...
					if (options?.capturesSystemAudio) {
						...
						tempSystemAudioPath = path.join(
							app.getPath("temp"),
							`RecorderTechoo-native-${timestamp}.system.wav`,
						);
					...
					if (options?.capturesMicrophone && !browserMicFallbackRequested) {
						...
						tempMicPath = path.join(
							app.getPath("temp"),
							`RecorderTechoo-native-${timestamp}.mic.wav`,
						);
			...
			try {
				...
				if (
					!ALLOW_RecorderTechoo_WINDOW_CAPTURE &&
					source?.id?.startsWith("window:") &&
					appName &&
					(appName === ownAppName || appName === "RecorderTechoo")
				) {
					return {
						success: false,
						message:
							"Cannot record RecorderTechoo windows. Please select another app window.",
					};
				}
				...
				if (
					...
					const { response } = await dialog.showMessageBox({
						...
						message:
							"RecorderTechoo needs screen recording permission to capture your screen.",
						detail: "Please open System Settings > Privacy & Security > Screen Recording, make sure RecorderTechoo is toggled ON, then try recording again.",
				...
				if (errorStr.includes("MICROPHONE_PERMISSION_DENIED")) {
					const { response } = await dialog.showMessageBox({
						...
						message: "RecorderTechoo needs microphone permission to record audio.",
						detail: "Please open System Settings > Privacy & Security > Microphone, make sure RecorderTechoo is toggled ON, then try recording again.",
						buttons: ["Open System Settings", "Cancel"],
						defaultId: 0,
						cancelId: 1,
					});
