	...
	ipcMain.handle("open-video-file-picker", async (_, options?: OpenVideoFilePickerOptions) => {
		try {
			...
			const result = await dialog.showOpenDialog({
				title: includeProjects ? "Import Media or RecorderTechoo Project" : "Select Video File",
				defaultPath: recordingsDir,
				filters: [
					...(includeProjects
						? [
								{
									name: "Media or RecorderTechoo Projects",
									extensions: [
					...
					...(includeProjects
						? [{ name: "RecorderTechoo Projects", extensions: PROJECT_FILE_EXTENSIONS }]
						: []),
