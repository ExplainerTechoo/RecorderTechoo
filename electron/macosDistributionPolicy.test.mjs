...
describe("macOS distribution architecture policy", () => {
	...
	it("uses path-specific helper architecture before the build architecture", () => {
		...
		expect(expectedMachOArchitecture("RecorderTechoo.app/Contents/MacOS/RecorderTechoo", "x64")).toBe(
			"x86_64",
		);
	});
	...
	it("reports a binary that lacks the required architecture", () => {
		expect(
			collectArchitectureErrors("RecorderTechoo.app/Contents/MacOS/RecorderTechoo", "arm64", "x64"),
		).toEqual(["RecorderTechoo.app/Contents/MacOS/RecorderTechoo does not contain x86_64 (found: arm64)"]);
	});
});
