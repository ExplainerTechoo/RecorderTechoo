...
const IS_SMOKE_EXPORT = process.env.RecorderTechoo_SMOKE_EXPORT === "1";
...
function closeEditorWindowToHud(window: BrowserWindow | null) {
	...
	// the editor, keeping RecorderTechoo in its ready-to-record state on the taskbar.
	window.hide();
...
function getPlatformAppIconFilename(size: 32 | 128 | 512) {
	const baseName = process.platform === "darwin" ? "RecorderTechoomac" : "RecorderTechoo";
...
function shouldUseTray() {
	// macOS and Windows expose RecorderTechoo through their Dock/taskbar. Keep the
...
function updateTrayMenu(recording: boolean = false) {
	...
	const trayToolTip = recording ? `Recording: ${selectedSourceName}` : "RecorderTechoo";
...
app.whenReady().then(async () => {
	if (process.platform === "win32") {
		app.setAppUserModelId("dev.recordertechoo.app");
	}
	...
	// RecorderTechoo does not use WebHID, Web Serial, or WebUSB. Do not grant devices by default.
	...
	if (IS_SMOKE_EXPORT || process.env.RecorderTechoo_DEV_OPEN_RECORDING_INPUT) {
		await logSmokeExportGpuDiagnostics();
		if (IS_SMOKE_EXPORT) {
			const smokeSource =
				process.env.RecorderTechoo_SMOKE_EXPORT_PROJECT ??
				process.env.RecorderTechoo_SMOKE_EXPORT_INPUT ??
				"<missing input>";
			console.log(`[smoke-export] Starting editor smoke export for ${smokeSource}`);
		} else {
			console.log(
				`[dev-open-recording] Starting editor for ${process.env.RecorderTechoo_DEV_OPEN_RECORDING_INPUT}`,
			);
		}
		createEditorWindowWrapper();
		return;
	...
	if (IS_DEV && process.env.RecorderTechoo_DEV_PREVIEW_UPDATE === "1") {
		setTimeout(() => {
			if (process.platform === "darwin") {
