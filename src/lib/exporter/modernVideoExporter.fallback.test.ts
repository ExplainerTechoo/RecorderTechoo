...
describe("ModernVideoExporter native fallback routing", () => {
	...
	it("builds actionable diagnostics for input decoder failures", () => {
		...
		exporter.runtimeDiagnostics = {
			appVersion: "1.4.0",
			userAgent: "RecorderTechooTest/1.0 Electron/43.1.0",
		...
		expect(report).toContain("RecorderTechoo version: 1.4.0");
		expect(report).toContain("Runtime: RecorderTechooTest/1.0 Electron/43.1.0");
