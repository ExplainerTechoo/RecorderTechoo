...
describe("getAudioResourceVersionKey", () => {
	it("changes identity when a finalized companion replaces a partial file at the same path", () => {
		const audioPath = "C:\\RecorderTechoo\\recording.mic.wav";
	...
	it("cache-busts only the trusted loopback media URL after finalization", () => {
		const localUrl = "http://127.0.0.1:43123/video?path=C%3A%5CRecorderTechoo%5Crecording.mic.wav";
		const versionedUrl = new URL(getVersionedAudioResourceUrl(localUrl, 2));

		expect(versionedUrl.searchParams.get("path")).toBe("C:\\RecorderTechoo\\recording.mic.wav");
		expect(versionedUrl.searchParams.get("RecorderTechooAudioVersion")).toBe("2");
	...
	it("uses one cache scope for every version of a loopback resource", () => {
		const baseUrl = "http://127.0.0.1:43123/video?path=C%3A%5CRecorderTechoo%5Crecording.mic.wav";
		const versionedUrl = `${baseUrl}&RecorderTechooAudioVersion=4`;

		expect(getAudioResourceCacheScope(versionedUrl)).toBe(baseUrl);
		expect(
			getAudioResourceCacheScope(
				"https://cdn.example/audio.wav?RecorderTechooAudioVersion=4&sig=abc",
			),
		).toBe("https://cdn.example/audio.wav?RecorderTechooAudioVersion=4&sig=abc");
	});
});
