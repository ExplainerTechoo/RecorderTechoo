...
const width = parseEvenInteger(process.env.RecorderTechoo_BENCH_EXPORT_WIDTH ?? "1280", "Width");
const height = parseEvenInteger(process.env.RecorderTechoo_BENCH_EXPORT_HEIGHT ?? "720", "Height");
const frameRate = parsePositiveInteger(process.env.RecorderTechoo_BENCH_EXPORT_FPS ?? "60", "Frame rate");
const durationSeconds = parsePositiveInteger(
	process.env.RecorderTechoo_BENCH_EXPORT_DURATION ?? "15",
	"Duration",
);
const timeoutMs = parsePositiveInteger(
	process.env.RecorderTechoo_BENCH_EXPORT_TIMEOUT_MS ?? "180000",
	"Timeout",
);
const runsPerVariant = parsePositiveInteger(process.env.RecorderTechoo_BENCH_EXPORT_RUNS ?? "2", "Runs");
const useNativeExport = process.env.RecorderTechoo_BENCH_EXPORT_USE_NATIVE === "1";
const useWebcamOverlay = process.env.RecorderTechoo_BENCH_EXPORT_ENABLE_WEBCAM === "1";
const providedInputPath = process.env.RecorderTechoo_BENCH_EXPORT_INPUT ?? null;
const providedWebcamInputPath = process.env.RecorderTechoo_BENCH_EXPORT_WEBCAM_INPUT ?? null;
const keepTempArtifacts = process.env.RecorderTechoo_BENCH_EXPORT_KEEP_TEMP === "1";
const exportEncodingMode = parseExportEncodingMode(
	process.env.RecorderTechoo_BENCH_EXPORT_ENCODING_MODE ?? null,
);
const exportQuality = parseExportQuality(process.env.RecorderTechoo_BENCH_EXPORT_QUALITY ?? null);
const exportShadowIntensity = parseExportShadowIntensity(
	process.env.RecorderTechoo_BENCH_EXPORT_SHADOW_INTENSITY ?? null,
);
const webcamWidth = parseEvenInteger(
	process.env.RecorderTechoo_BENCH_EXPORT_WEBCAM_WIDTH ?? "640",
	"Webcam width",
);
const webcamHeight = parseEvenInteger(
	process.env.RecorderTechoo_BENCH_EXPORT_WEBCAM_HEIGHT ?? "360",
	"Webcam height",
);
const webcamShadowIntensity = parseExportShadowIntensity(
	process.env.RecorderTechoo_BENCH_EXPORT_WEBCAM_SHADOW ?? null,
);
const webcamSize = parseExportWebcamSize(process.env.RecorderTechoo_BENCH_EXPORT_WEBCAM_SIZE ?? null);
const MODERN_BACKEND_SWEEP = ["auto", "webcodecs", "breeze"];
const exportPipeline = parseExportPipeline(process.env.RecorderTechoo_BENCH_EXPORT_PIPELINE ?? null);
const exportBackend = parseExportBackend(process.env.RecorderTechoo_BENCH_EXPORT_BACKEND ?? null);
const exportRenderBackend = parseRenderBackend(
	process.env.RecorderTechoo_BENCH_EXPORT_RENDER_BACKEND ?? null,
);
const exportBackendList = parseExportBackendList(
	process.env.RecorderTechoo_BENCH_EXPORT_BACKENDS ?? null,
);
...
const variantNameList = parseBenchmarkVariantList(
	process.env.RecorderTechoo_BENCH_EXPORT_VARIANTS ?? null,
);
...
function parseExportPipeline(rawValue) {
	...
	throw new Error("RecorderTechoo_BENCH_EXPORT_PIPELINE must be 'legacy' or 'modern'");
}
...
function parseExportBackend(rawValue) {
	...
	throw new Error("RecorderTechoo_BENCH_EXPORT_BACKEND must be 'auto', 'webcodecs', or 'breeze'");
}
...
function parseRenderBackend(rawValue) {
	...
	throw new Error("RecorderTechoo_BENCH_EXPORT_RENDER_BACKEND must be 'webgl' or 'webgpu'");
}
...
function parseExportBackendList(rawValue) {
	...
	if (values.length === 0) {
		throw new Error(
			"RecorderTechoo_BENCH_EXPORT_BACKENDS must include at least one of: auto, webcodecs, breeze",
		);
	}
...
function parseBenchmarkVariantList(rawValue) {
	...
	if (values.length === 0) {
		throw new Error(
			"RecorderTechoo_BENCH_EXPORT_VARIANTS must include at least one of: adaptive, baseline, tuned",
		);
	}
	...
	for (const value of values) {
		if (!(value in VARIANT_PRESETS)) {
			throw new Error(
				"RecorderTechoo_BENCH_EXPORT_VARIANTS must include only: adaptive, baseline, tuned",
			);
		}
	}
...
function parseExportEncodingMode(rawValue) {
	...
	throw new Error("RecorderTechoo_BENCH_EXPORT_ENCODING_MODE must be 'fast', 'balanced', or 'quality'");
}
...
function parseExportQuality(rawValue) {
	...
	throw new Error("RecorderTechoo_BENCH_EXPORT_QUALITY must be 'medium', 'good', 'high', or 'source'");
}
...
function parseExportShadowIntensity(rawValue) {
	...
	if (!Number.isFinite(parsed) || parsed < 0) {
		throw new Error("RecorderTechoo_BENCH_EXPORT_SHADOW_INTENSITY must be a non-negative number");
	}
...
function parseExportWebcamSize(rawValue) {
	...
	if (!Number.isFinite(parsed) || parsed <= 0 || parsed > 100) {
		throw new Error("RecorderTechoo_BENCH_EXPORT_WEBCAM_SIZE must be a number between 0 and 100");
	}
...
async function runVariant(
	...
	const child = spawn(electron, [repoRoot], {
		...
		env: {
			...process.env,
			RecorderTechoo_SMOKE_EXPORT: "1",
			RecorderTechoo_SMOKE_EXPORT_INPUT: inputPath,
			RecorderTechoo_SMOKE_EXPORT_OUTPUT: outputPath,
			...(useNativeExport ? { RecorderTechoo_SMOKE_EXPORT_USE_NATIVE: "1" } : {}),
			...(exportEncodingMode
				? { RecorderTechoo_SMOKE_EXPORT_ENCODING_MODE: exportEncodingMode }
				: {}),
			...(exportQuality ? { RecorderTechoo_SMOKE_EXPORT_QUALITY: exportQuality } : {}),
			...(exportShadowIntensity !== null
				? { RecorderTechoo_SMOKE_EXPORT_SHADOW_INTENSITY: String(exportShadowIntensity) }
				: {}),
			...(webcamInputPath ? { RecorderTechoo_SMOKE_EXPORT_WEBCAM_INPUT: webcamInputPath } : {}),
			...(webcamShadowIntensity !== null
				? { RecorderTechoo_SMOKE_EXPORT_WEBCAM_SHADOW: String(webcamShadowIntensity) }
				: {}),
			...(webcamSize !== null
				? { RecorderTechoo_SMOKE_EXPORT_WEBCAM_SIZE: String(webcamSize) }
				: {}),
			...(benchmarkRequest.pipeline
				? { RecorderTechoo_SMOKE_EXPORT_PIPELINE: benchmarkRequest.pipeline }
				: {}),
			...(benchmarkRequest.backend
				? { RecorderTechoo_SMOKE_EXPORT_BACKEND: benchmarkRequest.backend }
				: {}),
			...(exportRenderBackend
				? { RecorderTechoo_SMOKE_EXPORT_RENDER_BACKEND: exportRenderBackend }
				: {}),
			...(typeof variant.maxEncodeQueue === "number"
				? { RecorderTechoo_SMOKE_EXPORT_MAX_ENCODE_QUEUE: String(variant.maxEncodeQueue) }
				: {}),
			...(typeof variant.maxDecodeQueue === "number"
				? { RecorderTechoo_SMOKE_EXPORT_MAX_DECODE_QUEUE: String(variant.maxDecodeQueue) }
				: {}),
			...(typeof variant.maxPendingFrames === "number"
				? { RecorderTechoo_SMOKE_EXPORT_MAX_PENDING_FRAMES: String(variant.maxPendingFrames) }
				: {}),
		},
...
async function main() {
	...
	const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "RecorderTechoo-export-queue-bench-"));
