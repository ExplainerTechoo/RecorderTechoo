...
const bundledExePath = path.join(bundledDir, "RecorderTechoo-gpu-export.exe");
const helperId = "RecorderTechoo-gpu-export";
...
if (!cmake) {
	if (existsSync(bundledExePath)) {
		const verification = verifyNativeHelperManifest({
			...
			binaryName: "RecorderTechoo-gpu-export.exe",
		});
		if (!verification.ok) {
			console.error(
