...
const bundledExePath = path.join(bundledDir, "RecorderTechoo-nvidia-cuda-compositor.exe");
const helperId = "RecorderTechoo-nvidia-cuda-compositor";
const generatorArch = process.arch === "arm64" ? "ARM64" : "x64";
const videoCodecSdkRoot =
	process.env.RecorderTechoo_NVIDIA_VIDEO_CODEC_SDK_ROOT?.trim() ||
...
function fallbackToBundledHelperOrExit(reason) {
	if (existsSync(bundledExePath)) {
		const verification = verifyNativeHelperManifest({
			...
			binaryName: "RecorderTechoo-nvidia-cuda-compositor.exe",
		});
		if (!verification.ok) {
			console.warn(
...
if (!existsSync(path.join(videoCodecSdkRoot, "Samples", "NvCodec"))) {
	fallbackToBundledHelperOrExit(
		`NVIDIA Video Codec SDK samples not found at ${videoCodecSdkRoot}. Set RecorderTechoo_NVIDIA_VIDEO_CODEC_SDK_ROOT to build from source.`,
	);
}
...
function patchNvDecoderForRecorderTechooCallbacks() {
	...
	if (!header.includes("RecorderTechooMappedFrameHandler")) {
...
using RecorderTechooMappedFrameHandler = void (*)(CUdeviceptr, unsigned int, int, int, int, int64_t, void*);
using RecorderTechooDisplayFramePolicy = bool (*)(int, void*);
`,
    ...
    void SetMappedFrameHandler(RecorderTechooMappedFrameHandler handler, void* userData) { m_RecorderTechooMappedFrameHandler = handler; m_RecorderTechooMappedFrameUserData = userData; }
    void SetDisplayFramePolicy(RecorderTechooDisplayFramePolicy policy, void* userData) { m_RecorderTechooDisplayFramePolicy = policy; m_RecorderTechooDisplayFramePolicyUserData = userData; }
...
`,
    ...
    RecorderTechooMappedFrameHandler m_RecorderTechooMappedFrameHandler = nullptr;
    void* m_RecorderTechooMappedFrameUserData = nullptr;
    RecorderTechooDisplayFramePolicy m_RecorderTechooDisplayFramePolicy = nullptr;
    void* m_RecorderTechooDisplayFramePolicyUserData = nullptr;
`,
	...
	if (!source.includes("RecorderTechoo mapped frame callback")) {
    ...
    if (m_RecorderTechooDisplayFramePolicy &&
        !m_RecorderTechooDisplayFramePolicy(displayFrameIndex, m_RecorderTechooDisplayFramePolicyUserData))
    {
        NVDEC_API_CALL(cuvidUnmapVideoFrame(m_hDecoder, dpSrcFrame));
        return 1;
    }

    // RecorderTechoo mapped frame callback keeps the CUDA helper from making an
    // extra device-to-device copy when the caller can consume mapped NV12.
    if (m_RecorderTechooMappedFrameHandler)
    {
        m_RecorderTechooMappedFrameHandler(
            dpSrcFrame,
            nSrcPitch,
            m_nWidth,
            m_nHeight,
            m_nSurfaceHeight,
            pDispInfo->timestamp,
            m_RecorderTechooMappedFrameUserData);
        NVDEC_API_CALL(cuvidUnmapVideoFrame(m_hDecoder, dpSrcFrame));
        return 1;
    }
...
try {
	patchNvDecoderForRecorderTechooCallbacks();
} catch (error) {
	fallbackToBundledHelperOrExit(
...
try {
	configureWithWindowsCmakeGenerator({
		...
		configure: (generator, toolset) =>
			execSync(
				`${cmake} .. -G "${generator}" -A ${generatorArch}${toolset ? ` -T ${toolset}` : ""} -DRecorderTechoo_NVIDIA_VIDEO_CODEC_SDK_ROOT="${videoCodecSdkRoot}"`,
				{
					cwd: buildDir,
					stdio: "inherit",
					timeout: 120000,
...
const exePath = path.join(buildDir, "Release", "RecorderTechoo-nvidia-cuda-compositor.exe");
