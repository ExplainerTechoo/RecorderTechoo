...
const productName = packageJson.productName ?? packageJson.name ?? "RecorderTechoo";
const packageName = packageJson.name ?? "RecorderTechoo";
	...
	const executableCandidates =
		process.platform === "win32"
			? [
					...
					path.join(appDir, "RecorderTechoo.exe"),
				]
			: [
...
function getExpectedNativeHelperFiles(archTag) {
	if (archTag.startsWith("win32-")) {
		const helpers = [
			...
			{
				name: "RecorderTechoo-gpu-export.exe",
		...
		if (archTag === "win32-x64") {
			helpers.push({
				name: "RecorderTechoo-nvidia-cuda-compositor.exe",
	...
	if (archTag.startsWith("darwin-")) {
		return [
			{
				name: "RecorderTechoo-screencapturekit-helper",
				label: "ScreenCaptureKit helper",
				executable: true,
			},
			{ name: "RecorderTechoo-window-list", label: "Window list helper", executable: true },
			{ name: "RecorderTechoo-system-cursors", label: "System cursor helper", executable: true },
			{
				name: "RecorderTechoo-native-cursor-monitor",
